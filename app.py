"""
Variant of app.py that uses a YOLOSeg TFLite model for face detection.

- best_float32.tflite (YOLOSeg with 3 classes: person, animal, face)
  Only class id 2 ("face") is used here.

Other models used are the same as in app.py:
...
# YOLOSeg detector backend
from yolo_tflite import YOLOSegTFLite
# FaceMesh backend
...
# YOLO specifics
YOLO_FACE_CLASS_ID = 2
YOLO_NUM_CLASSES = 3
YOLO_IMG_SIZE = (384, 480)  # (H, W) used elsewhere in repo for this model
...
def classify_pose(yaw: float, pitch: float, roll: float) -> Optional[str]:
    if abs(pitch) > 170 and abs(yaw) < 30 and (abs(roll) < 15 or abs(roll) > 165):
        return "Pose: front"
    if abs(pitch) > 145 and yaw > 45 and (abs(roll) < 25 or abs(roll) > 155):
        return "Pose: left"
    if abs(pitch) > 145 and yaw < -45 and (abs(roll) < 25 or abs(roll) > 155):
        return "Pose: right"
    return None
...
def main():
    ...
    # Initialize YOLOSeg face detector (face class only)
    yolo = YOLOSegTFLite(
        "best_float32.tflite",
        imgsz=YOLO_IMG_SIZE,
        conf_thres=0.5,
    ...
    while True:
        ...
        # Face detection via YOLO (boxes in image coords)
        boxes_all, confs_all, clses_all, _masks = yolo.detect(frame)
        # keep only face detections (class id 2)
        if boxes_all.shape[0] > 0:
            face_mask = (clses_all == YOLO_FACE_CLASS_ID)
            boxes = boxes_all[face_mask]
            confs = confs_all[face_mask]
            clses = clses_all[face_mask]
        else:
        ...
        if boxes.shape[0] > 0:
            # Pick highest score face detection
            ...
            if min(bw, bh) < min_side_needed:
                ...
                cv2.imshow("Head Pose (YOLO) - press q to quit", frame)
